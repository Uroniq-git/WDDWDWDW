...
async def input_amount(call: CallbackQuery, state: FSMContext):
    ...
    if get_payment[5] == "True":
        ...
        await call.message.answer("<b>💸 Choose a deposit method</b>",
                                  reply_markup=choose_pay_type_func(call.from_user.id))
        await bot.delete_message(call.from_user.id, call.message.message_id)
    else:
        await bot.answer_callback_query(call.id, "❗ Bot deposits are temporarily disabled")
...
async def create_qiwi_pay(message: types.Message, state: FSMContext):
    if message.text.isdigit():
        ...
        del_msg = await bot.send_message(message.from_user.id, "<b>♻ Please wait, payment is being generated...</b>")
...
async def get_pay_type(call: CallbackQuery, state: FSMContext):
    ...
    if pay_type == "qiwi":
            ...
            if get_payment[0] != "None" and get_payment[1] != "None" and get_payment[2] != "None":
                try:
                    ...
                    if response_qiwi.status_code == 200:
                        await StorageQiwi.here_input_qiwi_amount.set()
                        await bot.delete_message(call.from_user.id, call.message.message_id)
                        await call.message.answer("<b>💵 Enter the amount to replenish funds in rub 🥝</b>",
                ...
                if check_pass:
                    await bot.answer_callback_query(call.id, "❗ Replenishment temporarily unavailable")
        ...
        await call.message.answer("💵 Enter the amount to replenish funds in rub 💎")
    await bot.delete_message(call.from_user.id, call.message.message_id)
...
async def create_crystal_pay(message: types.Message, state: FSMContext):
    if message.text.isdigit() and int(message.text) >= 0:
        ...
        link = crystal.generate_pay_link(message.text, redirect="@SlimyCloud_bot")
        await message.answer("🎈 Payment link generated:\n"
                             f"✔ ID payment: {link[0]}\n"
                             f"📎 Link: <a href='{link[1]}'>click to refill</a>",
        ...
        await StorageCrystal.here_input_crystal_amount.set()
        # await bot.delete_message(message.chat.id, del_msg.message_id)
        await message.answer(f"❌ <b>Invalid replenishment amount</b>\n"
                             f"▶ Min. replenishment amount: <code>{1}rub</code>\n"
                             f"💵 Enter the amount to replenish funds in rub 💎")
...
async def check_crystal_pay(call: CallbackQuery):
    ...
    if pay_status:
        ...
        if get_purchase is None:
            ...
            await call.message.delete()
            await call.message.answer(
                f"<b>✅ You have successfully replenished the balance in the amount {pay_amount}rub. Good luck ❤</b>\n"
                f"<b>📃 Check:</b> <code>+{receipt}</code>",
                reply_markup=check_user_out_func(call.from_user.id))
            await send_all_admin(f"<b>💎 User</b> "
                                 f"(@{call.from_user.username}|<a href='tg://user?id={call.from_user.id}'>{call.from_user.first_name}</a>"
                                 f"|<code>{call.from_user.id}</code>) "
                                 f"<b>Replenished the balance by the amount</b> <code>{pay_amount}rub</code> 🥝\n"
                                 f"📃 <b>Check:</b> <code>+{receipt}</code>")
        else:
            await bot.answer_callback_query(call.id, "❗ Your deposit has already been credited.", True)
    else:
